...
exports.UniversalChatService = void 0;
...
class UniversalChatService {
     ...
     */
    async getOrCreateChat(chatId, userId) {
        try {
            ...
            if (error && error.code !== 'PGRST116') {
                ...
                throw new error_middleware_1.AppError(`Failed to fetch chat room: ${error.message}`, 400);
            }
            if (!room) {
                ...
                const { data: newRoom, error: createError } = await this.supabase
                    ...
                    .single();
                if (createError) {
                    throw new error_middleware_1.AppError(`Failed to create chat room: ${createError.message}`, 400);
                }
                room = newRoom;
                ...
                await this.sendSystemMessage(chatId, this.generateWelcomeMessage(context, metadata));
                logger_1.logger.info(`Created new chat room: ${chatId}`);
            }
            return room;
        }
        catch (error) {
            logger_1.logger.error('Error in getOrCreateChat:', error);
            throw error;
        }
    }
    /**
    ...
    async buildMetadata(context) {
        ...
        try {
            ...
            logger_1.logger.error('Error building metadata:', error);
     ...
     */
    async autoSubscribe(chatId, context, currentUserId) {
        const users = new Set();
        ...
        try {
            ...
            switch (context.type) {
                case 'MTO':
                    if (context.mtoId) {
                        ...
                        const { data: mto } = await this.supabase
                            ...
                            .single();
                        if (mto) {
                            if (mto.brand_user_id)
                                users.add(mto.brand_user_id);
                            if (mto.factory_assignee_id)
                                users.add(mto.factory_assignee_id);
                        }
                    }
                    else if (context.date) {
                ...
                case 'PO':
                    ...
                    const { data: po } = await this.supabase
                        ...
                        .single();
                    if (po) {
                        if (po.brand_buyer_id)
                            users.add(po.brand_buyer_id);
                        if (po.factory_manager_id)
                            users.add(po.factory_manager_id);
                    }
                    break;
            }
            ...
            logger_1.logger.info(`Auto-subscribed ${subscriptions.length} users to chat ${chatId}`);
        }
        catch (error) {
            logger_1.logger.error('Error in autoSubscribe:', error);
    ...
    async sendMessage(chatId, userId, content, type = 'text') {
        try {
            const { data: message, error } = await this.supabase
                ...
                .single();
            if (error) {
                throw new error_middleware_1.AppError(`Failed to send message: ${error.message}`, 400);
            }
    ...
    async getMessages(chatId, limit = 50, offset = 0) {
        ...
        `)
            ...
            if (error) {
                throw new error_middleware_1.AppError(`Failed to fetch messages: ${error.message}`, 400);
            }
            return messages || [];
        }
        catch (error) {
    ...
    async getRelatedChats(chatId) {
        ...
        try {
            ...
            logger_1.logger.error('Error getting related chats:', error);
        }
        return related;
    }
    /**
     ...
     */
    async navigateChat(currentChatId, direction) {
        ...
        try {
            ...
            logger_1.logger.error('Error navigating chat:', error);
        }
        return targets;
    }
    /**
    ...
    async getParticipants(chatId) {
        try {
            ...
            logger_1.logger.error('Error fetching participants:', error);
            return [];
        }
    }
    /**
     ...
     */
    generateWelcomeMessage(context, metadata) {
...
exports.UniversalChatService = UniversalChatService;
exports.default = UniversalChatService;
//# sourceMappingURL=universal-chat.service.js.map