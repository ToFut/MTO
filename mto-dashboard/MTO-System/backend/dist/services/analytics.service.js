...
class AnalyticsService {
    ...
    async getDashboardOverview(filters) {
        try {
            ...
            const onTimeCount = deliveries?.filter(d => new Date(d.actual_ship_date) <= new Date(d.expected_ship_date)).length || 0;
            ...
            logger_1.logger.error('Dashboard overview error:', error);
            throw new error_middleware_1.AppError('Failed to fetch dashboard overview', 500);
        }
    }
    async getProductionAnalytics(filters) {
        try {
            ...
            // Group by date
            const grouped = mtos?.reduce((acc, mto) => {
            ...
            const totalProduced = analytics.reduce((sum, day) => sum + day.completed, 0);
            const totalTarget = analytics.reduce((sum, day) => sum + day.total, 0);
            ...
            logger_1.logger.error('Production analytics error:', error);
            throw new error_middleware_1.AppError('Failed to fetch production analytics', 500);
        }
    }
    async getTopProducts(filters) {
        try {
            ...
            const productMap = mtos?.reduce((acc, mto) => {
            ...
            logger_1.logger.error('Top products error:', error);
            throw new error_middleware_1.AppError('Failed to fetch top products', 500);
        }
    }
    async getDefectAnalytics(filters) {
        try {
            ...
            // Group by stage
            const byStage = defects?.reduce((acc, defect) => {
                const stage = defect.production_stage || 'Unknown';
                acc[stage] = (acc[stage] || 0) + 1;
                return acc;
            }, {});
            // Trend over time
            const trend = defects?.reduce((acc, defect) => {
            ...
            logger_1.logger.error('Defect analytics error:', error);
            throw new error_middleware_1.AppError('Failed to fetch defect analytics', 500);
        }
    }
    async getShippingAnalytics(filters) {
        try {
            ...
            logger_1.logger.error('Shipping analytics error:', error);
            throw new error_middleware_1.AppError('Failed to fetch shipping analytics', 500);
        }
    }
    async getInventoryAnalytics(filters) {
        try {
            ...
            const totalValue = inventory?.reduce((sum, item) => sum + (item.quantity_available || 0) * (item.unit_price || 0), 0) || 0;
            // Identify shortages
            const shortage = inventory?.filter(item => item.quantity_available <= item.reorder_point).map(item => ({
                sku: item.sku_code,
                name: item.item_name,
            ...
            logger_1.logger.error('Inventory analytics error:', error);
            throw new error_middleware_1.AppError('Failed to fetch inventory analytics', 500);
        }
    }
    async getTimelineView(filters) {
        try {
            ...
            logger_1.logger.error('Timeline view error:', error);
            throw new error_middleware_1.AppError('Failed to fetch timeline view', 500);
        }
    }
    async getPerformanceMetrics(filters) {
        try {
            ...
            logger_1.logger.error('Performance metrics error:', error);
            throw new error_middleware_1.AppError('Failed to fetch performance metrics', 500);
        }
    }
    async getEfficiencyReport(filters) {
        try {
            ...
            const byStage = mtos?.reduce((acc, mto) => {
            ...
            logger_1.logger.error('Efficiency report error:', error);
            throw new error_middleware_1.AppError('Failed to fetch efficiency report', 500);
        }
    }
    async getQualityMetrics(filters) {
        try {
            ...
            logger_1.logger.error('Quality metrics error:', error);
            throw new error_middleware_1.AppError('Failed to fetch quality metrics', 500);
        }
    }
    async getZipCodeAnalytics(filters) {
        try {
            ...
            logger_1.logger.warn('Zip code analytics requires zip_code field in database');
            return {
                byZip: {},
                heatmap: []
            };
            ...
            logger_1.logger.error('Zip code analytics error:', error);
            throw new error_middleware_1.AppError('Failed to fetch zip code analytics', 500);
        }
    }
    async getTrendAnalysis(filters) {
        try {
            ...
            const { data: mtos } = await query;
            // Group by day
            const dailyData = mtos?.reduce((acc, mto) => {
            ...
            const trend = Object.values(dailyData || {}).sort((a, b) => new Date(a.date).getTime() - new Date(b.date).getTime());
            ...
            logger_1.logger.error('Trend analysis error:', error);
            throw new error_middleware_1.AppError('Failed to fetch trend analysis', 500);
        }
    }
    async getForecast(filters) {
        try {
            ...
            const forecast = trendData.forecast.map((point) => ({
                ...point,
            ...
            logger_1.logger.error('Forecast error:', error);
            throw new error_middleware_1.AppError('Failed to generate forecast', 500);
        }
    }
    async generateCustomReport(options) {
        try {
            ...
            logger_1.logger.error('Custom report generation error:', error);
            throw new error_middleware_1.AppError('Failed to generate custom report', 500);
        }
    }
    async saveReportTemplate(templateData) {
        try {
            ...
            logger_1.logger.error('Save report template error:', error);
            throw new error_middleware_1.AppError('Failed to save report template', 500);
        }
    }
    async getReportTemplates(companyId) {
        try {
            ...
            logger_1.logger.error('Get report templates error:', error);
            throw new error_middleware_1.AppError('Failed to fetch report templates', 500);
        }
    }
    async deleteReportTemplate(templateId, userId) {
        try {
            ...
            logger_1.logger.error('Delete report template error:', error);
            throw new error_middleware_1.AppError('Failed to delete report template', 500);
        }
    }
}
