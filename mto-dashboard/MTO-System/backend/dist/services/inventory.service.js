...
class InventoryService {
     ...
     */
    async autoPopulateFromMTOs(mtos, brandId, factoryId) {
        try {
            logger_1.logger.info(`Starting smart inventory auto-population from ${mtos.length} MTOs...`);
            ...
            for (let i = 0; i < inventoryItems.length; i += batchSize) {
                ...
                for (const item of batch) {
                    try {
                        ...
                        logger_1.logger.error(`Failed to process inventory item ${item.item_code}:`, error);
                    }
                }
            }
            ...
            logger_1.logger.info(`Inventory auto-population completed: ${createdCount} created, ${updatedCount} updated`);
            return {
            ...
            logger_1.logger.error('Error in smart inventory auto-population:', error);
            throw error;
        }
    }
     ...
     */
    calculateInventoryPriority(mto, spot) {
        let priorityScore = 0;
        // MTO priority influence
        if (mto.priority === 'urgent')
            priorityScore += 3;
        else if (mto.priority === 'high')
            priorityScore += 2;
        else if (mto.priority === 'normal')
            priorityScore += 1;
        ...
        if (spot.complexity_level === 'complex')
            priorityScore += 1;
     ...
     * Generate shortage alerts for high-priority items
     */
    async generateShortageAlerts(inventoryItems, brandId, factoryId) {
        const alerts = [];
        ...
        logger_1.logger.info(`Generated ${alerts.length} shortage alerts`);
        return alerts;
    }
}
