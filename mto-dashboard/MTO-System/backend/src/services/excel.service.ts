...
export interface ParsedMTO {
  ...
  // Dynamic spots array (flexible for 3, 6, 20+ spots)
  spots: SpotData[];
...
export class ExcelService {
   ...
   */
  async analyzeExcelStructure(fileBuffer: Buffer): Promise<ExcelAnalysisResult> {
    try {
      logger.info('Starting smart Excel structure analysis...');
      ...
      logger.info(`Excel analysis complete: ${analysis.totalRows} rows, ${analysis.detectedSpots} spots detected`);
      return analysis;

    } catch (error) {
      logger.error('Excel analysis failed:', error);
   ...
   */
  async extractMTOsFromExcel(analysisResult: ExcelAnalysisResult, fileBuffer?: Buffer): Promise<ParsedMTO[]> {
    try {
      if (!fileBuffer) {
        throw new Error('File buffer required for MTO extraction');
      }

      logger.info('Starting MTO extraction from Excel...');
      ...
      logger.info(`Extracted ${mtos.length} MTOs from Excel`);
      return mtos;

    } catch (error) {
      logger.error('MTO extraction failed:', error);
   ...
   */
  private performStructuralAnalysis(rawData: any[][]): ExcelAnalysisResult {
   ...
   */
  private detectHeaders(rawData: any[][]): boolean {
    if (!rawData[0] || !rawData[1]) return false;
   ...
   */
  private detectColumnPatterns(headerRow: any[], hasHeaders: boolean): Record<string, string> {
   ...
   */
  private detectSpots(sampleRows: any[][], columnMapping: Record<string, string>): {
    detectedSpots: number;
    spotColumns: Array<{
      position: number;
      skuColumn: string;
    ...
    for (let i = 0; i < spotCandidates.length; i++) {
      ...
      const hasSpotData = sampleRows.some(row => 
        row[currentCol] && this.looksLikeSKU(row[currentCol])
      );
      
      if (hasSpotData) {
        const spotColumn: any = {
          position: position++,
        ...
        if (nextCol && nextCol === currentCol + 1) {
          const hasRefData = sampleRows.some(row => 
            row[nextCol] && this.looksLikePatchRef(row[nextCol])
          );
   ...
   */
  private findSpotsByDataPattern(sampleRows: any[][]): number[] {
    const candidates: number[] = [];
    
    if (!sampleRows.length) return candidates;
   ...
   */
  private looksLikeSKU(value: any): boolean {
    ...
    return (
      /^\d{4,8}$/.test(str) ||  // 4-8 digit numbers (common for SKUs)
   ...
   */
  private looksLikePatchRef(value: any): boolean {
    if (!value) return false;
   ...
   */
  private extractMTOsFromRows(rawData: any[][], analysis: ExcelAnalysisResult): ParsedMTO[] {
    const mtos: ParsedMTO[] = [];
    const startRow = analysis.hasHeaders ? 1 : 0;
    
    for (let rowIndex = startRow; rowIndex < rawData.length; rowIndex++) {
      const row = rawData[rowIndex];
      if (!row || !this.isValidDataRow(row)) continue;
      ...
      try {
        ...
        logger.warn(`Failed to extract MTO from row ${rowIndex + 1}:`, error);
      }
    }
   ...
   */
  private extractMTOFromRow(row: any[], analysis: ExcelAnalysisResult, rowIndex: number): ParsedMTO | null {
    // Extract core fields
    const mto: any = {
  ...
  private suggestOptimalMapping(
    ...
    spotAnalysis.spotColumns.forEach((spotCol: any, index: number) => {
