...
export class MTOService {
  ...
  async previewMTOUpload(
    ...
    try {
      ...
      let parsedMTOs: any[] = [];
      let poData: any = null;
      let excelAnalysis: any = { detectedSpots: 0, qualityScore: 0, totalRows: 0 };

      if (fileType === 'PO') {
        // Handle Bauble Bar PO format
        logger.info('Processing as Bauble Bar PO format...');
      ...
      enrichedMTOs.forEach(mto => {
        ...
        if (mto.bag_base_pid) inventoryItems.add(mto.bag_base_pid);
   ...
   */
  private generateUploadWarnings(mtos: any[], analysis: any): string[] {
  ...
  async directMTOUpload(
    ...
    try {
      logger.info('Starting direct MTO upload process');
      ...
      logger.info(`Parsed ${validMTOs.length} valid MTOs from Excel`);
      ...
      if (poError) {
        logger.error('PO creation failed:', poError);
        throw new Error(`Failed to create purchase order: ${poError.message}`);
      }

      logger.info(`Created PO: ${purchaseOrder.po_number}`);
      ...
      if (internalIds.length !== uniqueIds.size) {
        logger.error('Duplicate internal_id values found within batch:', internalIds);
        ...
        for (let i = 0; i < finalMTOData.length; i++) {
          ...
          if (count > 0) {
            ...
            logger.info(`Replacing duplicate internal_id "${id}" with "${newId}"`);
      ...
      if (checkError) {
        logger.error('Error checking existing internal_ids:', checkError);
      } else if (existing && existing.length > 0) {
        logger.error('Found existing internal_ids in database:', existing.map(m => m.internal_id));
      ...
      if (mtoError) {
        logger.error('MTO save failed:', mtoError);
        throw new Error(`Failed to save MTOs: ${mtoError.message}`);
      }

      logger.info(`Saved ${savedMTOs.length} MTOs to database`);
      ...
      logger.error('Direct MTO upload failed:', error);
      throw error;
    }
  }
  ...
  async smartUploadMTOs(
    ...
    try {
      logger.info(`Starting smart MTO upload for PO: ${poNumber}`);
      ...
      if (fileType === 'PO') {
        ...
        logger.info(`Parsed ${parsedMTOs.length} MTOs from PO with ${poData.lineItems.length} line items`);
      ...
      const analysisReport = this.generateSmartAnalysisReport(
        savedMTOs, 
        autoPopulationResults,
        excelAnalysis
      );

      logger.info(`Smart MTO upload completed: ${savedMTOs.length} MTOs processed`);
      ...
      logger.error('Smart MTO upload failed:', error);
      throw error;
    }
  }
  ...
  // Helper methods
  private async detectFileType(fileBuffer: Buffer): Promise<'PO' | 'MTO'> {
    try {
      ...
      for (let i = 0; i < Math.min(20, data.length); i++) {
        ...
        poIndicators.forEach(indicator => {
          if (rowText.includes(indicator)) poScore++;
        });
        
        mtoIndicators.forEach(indicator => {
          if (rowText.includes(indicator)) mtoScore++;
        });
      }
      
      logger.info(`File type detection - PO score: ${poScore}, MTO score: ${mtoScore}`);
      
      return poScore > mtoScore ? 'PO' : 'MTO';
    } catch (error) {
      logger.warn('Could not detect file type, defaulting to MTO:', error);
      return 'MTO';
    }
  }
  ...
  private generateUniqueInternalId(rowIndex?: number): string {
  ...
  private generateUniqueReferenceNumber(rowIndex?: number): string {
    const timestamp = Date.now();
    const hrtime = process.hrtime.bigint();
    const randomBytes = crypto.randomBytes(4).toString('hex');
    const rowSuffix = rowIndex !== undefined ? `-${rowIndex}` : '';
  ...
  private async createMTOBarcodes(mtoId: string, mtoData: any) {
    try {
      ...
      if (mtoData.spots_data && Array.isArray(mtoData.spots_data)) {
        mtoData.spots_data.forEach((spot: any) => {
          if (spot.sku) {
            barcodes.push({
              mto_id: mtoId,
              type: 'spot',
   ...
   */
  private async parseExcelData(fileBuffer: Buffer) {
    try {
      logger.info(`Starting Excel parse, buffer size: ${fileBuffer.length} bytes`);
      ...
      for (const sheetName of workbook.SheetNames) {
        ...
        if (!data || data.length < 2) {
          logger.info(`Sheet ${sheetName} is empty or has no data rows, skipping`);
          continue;
        }
        ...
        for (let i = 0; i < Math.min(10, data.length); i++) {
          ...
          if (row && row.length > 0) {
            ...
            if (mappedFields >= 2) {
              headerRowIndex = i;
              headers = row;
              logger.info(`Found header row at index ${i} in sheet ${sheetName}`);
              break;
            }
          }
        }
        
        logger.info(`Headers in sheet ${sheetName}:`, headers);
        const headerMap = this.createHeaderMap(headers);
        logger.info(`Header mapping for sheet ${sheetName}:`, headerMap);
        
        // Process data rows in batches for large files
        const dataRows = data.slice(headerRowIndex + 1);
        logger.info(`Processing ${dataRows.length} data rows from sheet ${sheetName}`);
        ...
        for (let batchStart = 0; batchStart < dataRows.length; batchStart += BATCH_SIZE) {
          ...
          logger.info(`Processing batch ${Math.floor(batchStart / BATCH_SIZE) + 1}: rows ${batchStart + 1}-${batchEnd} of ${dataRows.length}`);
          
          for (let batchIndex = 0; batchIndex < batch.length; batchIndex++) {
            const i = batchStart + batchIndex + headerRowIndex + 1; // Actual row number in original data
            const row = batch[batchIndex] as any[];
        ...
        try {
          ...
          if (internalId && (internalId.toLowerCase() === 'no' || internalId.toLowerCase() === 'yes')) {
            logger.warn(`Invalid internal_id value "${internalId}" at row ${i + 1}, will generate new ID`);
            internalId = null;
          }
          ...
          if (!internalId || usedInternalIds.has(internalId)) {
            if (internalId && usedInternalIds.has(internalId)) {
              logger.warn(`Duplicate internal_id "${internalId}" found at row ${i + 1}, generating new ID`);
            }
          ...
          if (!displayName || displayName.toString().trim() === '') {
            ...
            for (const field of fallbackFields) {
              ...
              if (value && value.toString().trim() !== '') {
                displayName = value;
                logger.info(`Using ${field} as display name: ${displayName}`);
                break;
              }
            }
            ...
            if (!displayName || displayName.toString().trim() === '') {
              ...
              logger.info(`Generated display name: ${displayName}`);
            }
          }
          ...
          if (!mtoData.display_name || mtoData.display_name.trim() === '') {
            logger.warn(`Row ${i + 1} has no identifiable data, skipping`);
            errors.push({
              row: i + 1,
          ...
          if (!hasSpots && !hasQuantity) {
            logger.warn(`Row ${i + 1} appears to be empty or invalid, skipping`);
        ...
        if (batchEnd < dataRows.length) {
          logger.info(`Batch complete: ${validMTOs.length} valid MTOs so far, ${errors.length} errors`);
        }
      ...
      logger.info(`Parsed ${allValidMTOs.length} valid MTOs from ${workbook.SheetNames.length} sheets`);
   ...
   */
  private parseExcelDataLegacy(data: any[], poId: string, brandId: string): any[] {
  ...
  private parseExcelDate(value: any): string | null {
    ...
    try {
      ...
      if (typeof value === 'number') {
        ...
        if (year < 1900 || year > 2100 || isNaN(date.getTime())) return null;
      ...
      if (typeof value === 'string') {
        ...
        // Sanity check
        if (isNaN(parsed.getTime()) || year < 1900 || year > 2100) return null;
   ...
   */
  private async createOrGetPO(poNumber: string, brandId: string, factoryId: string) {
   ...
   */
  private async smartEnrichMTOs(parsedMTOs: any[], brandId: string, factoryId: string) {
    const enrichedMTOs = [];
    ...
    for (const parsedMTO of parsedMTOs) {
      try {
        ...
        logger.error(`Failed to enrich MTO ${parsedMTO.internal_id}:`, error);
   ...
   */
  private async processAndEnrichSpots(spots: any[], brandId: string, factoryId: string) {
    ...
    for (const spot of spots) {
      try {
        ...
        const vocabularyMapping = await this.vocabularyService.findOrCreateMapping(
          spot.sku,
          brandId,
          factoryId,
          spot.patch_ref
        ...
        logger.error(`Failed to enrich spot ${spot.sku}:`, error);
   ...
   */
  private async bulkCreateMTOsWithSpots(enrichedMTOs: any[], poId: string, userId: string) {
    const savedMTOs = [];
    ...
    for (let i = 0; i < enrichedMTOs.length; i += batchSize) {
      ...
      if (error) {
        logger.error(`Failed to insert MTO batch ${i}-${i + batchSize}:`, error);
        throw new AppError(error.message, 400);
      }
    ...
    logger.info(`Successfully saved ${savedMTOs.length} MTOs to database`);
    return savedMTOs;
  }
   ...
   */
  private async autoPopulateAllSystems(savedMTOs: any[], brandId: string, factoryId: string, userId: string) {
    logger.info('Starting auto-population of all systems...');
   ...
   */
  private async processVocabularyForMTOs(mtos: any[], brandId: string, factoryId: string) {
    try {
      ...
      logger.info(`Vocabulary processing complete: ${created} created, ${translated} translated`);
      return { created, translated, mappings: Array.from(vocabularyMap.values()) };
    } catch (error) {
      logger.error('Error processing vocabulary:', error);
   ...
   */
  private detectPatchType(description: string | null): string {
    if (!description) return 'embroidery';
    
    const desc = description.toLowerCase();
    if (desc.includes('letter')) return 'letter';
    if (desc.includes('icon')) return 'icon';
    if (desc.includes('logo')) return 'logo';
    if (desc.includes('text')) return 'text';
    if (desc.includes('emoji')) return 'emoji';
   ...
   */
  private async generateComprehensiveBarcodes(mtos: any[]) {
    try {
      const barcodes = [];
      ...
      if (barcodes.length > 0) {
        ...
        if (error) {
          logger.error('Error creating barcodes:', error);
          return { created: 0, error: error.message };
        }

        logger.info(`Created ${inserted?.length || 0} barcodes`);
   ...
   */
  private generateSmartAnalysisReport(savedMTOs: any[], autoPopulationResults: any, excelAnalysis: any) {
    ...
    savedMTOs.forEach(mto => {
      if (mto.spots_data) {
        mto.spots_data.forEach((spot: any) => uniqueSpots.add(spot.sku));
      }
    });
    ...
    const spotPatterns: Record<string, number> = {};
    savedMTOs.forEach(mto => {
   ...
   */
  private inferVisualLocation(position: number): string {
   ...
   */
  private estimateProductionTime(patchRef: string | null): number {
    if (!patchRef) return 15; // Default 15 minutes
    
    const ref = patchRef.toLowerCase();
    
    // Icon patches (simple)
    if (ref.includes('icon')) return 10;
    
    // Letter patches (medium)
    if (ref.includes('letter') || ref.includes(' - ')) return 15;
    
    // Complex text or designs
    if (ref.length > 20) return 25;
   ...
   */
  private isRushOrder(shipDate: Date | string | null): boolean {
   ...
   */
  private generateSmartTags(mto: any, spots: any[]): string[] {
    const tags: string[] = [];
    ...
    if (mto.is_rush) tags.push('rush');
    
    // Add product type tags
    if (mto.display_name) {
      const name = mto.display_name.toLowerCase();
      if (name.includes('tote')) tags.push('tote-bag');
      if (name.includes('custom')) tags.push('custom');
      if (name.includes('medium')) tags.push('medium-size');
      if (name.includes('large')) tags.push('large-size');
    }
  ...
  private createHeaderMap(headers: string[]): Record<string, number> {
    ...
    headers.forEach((header, index) => {
      ...
      for (const [field, variations] of Object.entries(mappings)) {
        ...
        if (variations.some(variation => 
          ...
          map[field] = index;
          logger.info(`Mapped column "${header}" at index ${index} to field "${field}"`);
          break;
        }
      }
    });
    ...
    requiredFields.forEach(field => {
      if (!(field in map) || map[field] === -1) {
        logger.warn(`Required field "${field}" not found in headers`);
      }
    });
   ...
   */
  private getColumnValue(row: any[], headerMap: Record<string, number>, field: string): any {
   ...
   */
  private captureAllExcelData(row: any[], headers: string[], headerMap: Record<string, number>, metadata: any): any {
    const excelData: any = { ...metadata };
    ...
    for (let i = 1; i <= 6; i++) {
      const trackerHeader = headers.find(h => 
        h && h.toLowerCase().includes(`tracker`) && h.includes(`${i}`)
      );
      if (trackerHeader) {
    ...
    excelData.non_empty_columns = Object.keys(excelData).filter(k => 
      !['original_headers', 'mapped_fields', 'spots', 'tracker_customizations', ...Object.keys(metadata)].includes(k)
    ).length;
   ...
   */
  private async performAutoPopulation(savedMTOs: any[], factoryId: string) {
    try {
      logger.info('Starting auto-population process...');
      ...
      logger.info(`Auto-population completed: ${results.inventory.created} inventory items, ${results.barcodes.created} barcodes`);
