...
export class VocabularyService {
  ...
  async findOrCreateMapping(
    ...
    try {
      ...
      logger.error(`Error in findOrCreateMapping for SKU ${sku}:`, error);
      return null;
    }
  }
   ...
   */
  async createMappingsFromMTOs(mtos: any[], brandId: string, factoryId: string): Promise<any> {
    try {
      logger.info('Creating vocabulary mappings from MTOs...');
        ...
        if (mto.spots_data) {
          for (const spot of mto.spots_data) {
            if (spot.sku && !skusProcessed.has(spot.sku)) {
              ...
              if (!existing) {
                const mapping = this.generateSmartMappingData(
                  spot.sku, 
                  brandId, 
                  factoryId, 
                  spot.patch_ref, 
      ...
      if (mappingsToCreate.length > 0) {
        ...
        if (error) {
          logger.error('Error creating vocabulary mappings:', error);
        } else {
          createdCount = created?.length || 0;
        }
      }

      logger.info(`Created ${createdCount} new vocabulary mappings from ${mtos.length} MTOs`);
      ...
      logger.error('Error creating mappings from MTOs:', error);
      throw error;
    }
  }
   ...
   */
  private async createSmartMapping(
    sku: string, 
    brandId: string, 
    factoryId: string, 
    patchRef?: string
    ...
    try {
      ...
      if (error) {
        logger.error('Error creating smart mapping:', error);
        return null;
      }

      logger.info(`Created smart mapping for SKU ${sku}`);
      return data;

    } catch (error) {
      logger.error('Error in createSmartMapping:', error);
      return null;
    }
  }
   ...
   */
  private analyzePatchReference(patchRef?: string): any {
    if (!patchRef) {
    ...
    if (ref.includes('front')) analysis.visualLocation = 'front_center';
    else if (ref.includes('back')) analysis.visualLocation = 'back_center';
    else if (ref.includes('side')) analysis.visualLocation = 'side_left';
    else if (ref.includes('handle')) analysis.visualLocation = 'handle';
   ...
   */
  private findBestSimilarityMatch(sku: string, patchRef: string = '', mappings: any[]): {
    mapping: any;
    score: number;
  } {
